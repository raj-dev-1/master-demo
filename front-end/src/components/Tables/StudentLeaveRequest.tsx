...
const StudentLeaveRequest = () => {
  ...
  console.log("faculty",user.profile.id);
  useEffect(() => {
    const getApi = async () => {
      try {
        const result = await getApiCall(`/leave/allLeaveStatus/?search=${searchValue || selectValue}&page=${currentPage}&limit=100`);
        const filteredData : any = result?.data?.leaveStatus.filter(item => item.requestedTo.id === user.profile.id);
    ...
    getApi();
  }, [selectValue, searchValue,currentPage]);
            ...
            <table className="text-gray-500 dark:text-gray-400 w-full text-left text-sm">
              <thead className="text-gray-700 dark:text-gray-400 border border-stroke bg-primary text-xs uppercase text-white dark:border-form-strokedark dark:bg-strokedark">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3">
                    Requested By
                  </th> <th scope="col" className="px-4 py-3">
                    Leave Type
                  </th>
                  <th scope="col" className="px-4 py-3">
                    Reason
