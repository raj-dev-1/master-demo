...
const page = () => {
  ...
  const InitialValues: { email: string; otp: number | null } = {
    email: emailLocal || "",
    otp: null,
  };
  ...
  const {
    ...
    validationSchema: Yup.object({
      ...
      otp: Yup.number()
        .typeError("OTP should be a number") // This will trigger if the input is not a number
        .min(4, "Number should be at least 4")
        .required("Please enter a number"),
    }),
    onSubmit: async () => {
      const result = await postApiCall("/auth/verifyOtp", values);
      console.log(result);
            ...
            <form className="mt-8" onSubmit={handleSubmit}>
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="text-gray-400 focus-within:text-gray-600 relative mt-2.5">
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                      <TbPasswordMobilePhone className="h-5 w-5"/>
                    </div>
                  ...
                  {errors.otp && touched.otp ? (
                    <p className="mt-2 text-sm text-danger">{errors.otp}</p>
                  ) : (
                    ""
                  )}
                </div>
